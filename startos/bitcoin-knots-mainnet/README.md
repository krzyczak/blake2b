...
The node binary does not come from a registry. The repo's own `Dockerfile` downloads the signed RC4 test-release tarball from `test.bitcoinknots.org`, verifies it, and copies `bitcoind` and `bitcoin-cli` onto a slim Debian base. Three registry images run alongside it.
