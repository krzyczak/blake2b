<p align="center">
  <img src="icon.svg" alt="Mempool Guide Logo" width="21%">
</p>

# Mempool Guide on StartOS
...
Two images are reproducibly built from the pinned Retropex source, and MariaDB is pulled from its upstream image. They run as four daemons in a fixed order.

| Property      | Value                                            |
| ------------- | ------------------------------------------------ |
| Images        | locally built Retropex frontend/backend, `mariadb` |
...
Four volumes are declared and all carry data.
...
**Enforced** — rewritten to a fixed value whenever the package writes the file: the whole `DATABASE` section bar the password (the bundled MariaDB on loopback), `MEMPOOL.BACKEND`, `MEMPOOL.AUTOMATIC_POOLS_UPDATE`, the two `MEMPOOL.POOLS_JSON` URLs (the bundled snapshot on loopback), `CORE_RPC.COOKIE_PATH`, LND's certificate and macaroon paths, Core Lightning's socket path, and the `SYSLOG`, `MAXMIND`, `REDIS`, `REPLICATION`, and `STRATUM` sections, all held off.
...
| `MEMPOOL.AUTOMATIC_POOLS_UPDATE`                 | true               | false              | Imports changed bundled identities after a package upgrade and reindexes blocks still assigned to Unknown                                  |
...
- **What it changes:** `torProxy` in `store.json`, and through it `SOCKS5PROXY` in `mempool-config.json` and the package's optional Tor dependency. The pools snapshot is unaffected — upstream's pools updater installs its SOCKS agent as `httpsAgent` only, so the plain-http loopback fetch is exempt from the proxy and keeps working.
...
8. **x86_64 only.** This sideload build targets the user's StartOS hardware and does not include aarch64 or riscv64 images.
9. **Mining-pool identities update with package releases, not directly from GitHub.** A changed bundled snapshot is imported on startup and blocks assigned to Unknown are reindexed. Definitions committed after the installed package was built require a newer package release.
...
```yaml
package_id: mempool-guide
images: locally built Retropex frontend/backend, plus mariadb
architectures:
  - x86_64
subcontainers:
